...
import { money, num, JOB_TYPE_OPTIONS, MONTH_OPTIONS } from '../lib';

export function ConvertModal({ saleId, onClose }: { saleId: number; onClose: () => void }) {
  ...
  const doConvert = async () => {
    await store.convertSale(saleId, form);
    onClose();
  };
  ...
  return (
    <Modal
      ...
      subtitle="Creates a client record and adds the job to your dashboard"
      onClose={onClose}
      maxWidth={600}
      footer={
        <>
          ...
          <button onClick={doConvert} style={{ display: 'inline-flex', alignItems: 'center', gap: 7, padding: '9px 17px', border: 'none', borderRadius: 8, background: accent, color: '#fff', fontSize: 13, fontWeight: 600, cursor: 'pointer', boxShadow: '0 1px 2px rgba(15,30,44,.14)' }}><Icon name="check" size={16} />Convert &amp; add job</button>
        </>
      }
    >
      <div style={{ padding: '17px 20px', display: 'flex', flexDirection: 'column', gap: 14, maxHeight: '64vh', overflowY: 'auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 13, padding: '14px 16px', background: soft, borderRadius: 8 }}>
          <span style={{ width: 38, height: 38, borderRadius: 8, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#fff', color: accent }}><Icon name="user-plus" size={19} /></span>
          <div style={{ minWidth: 0 }}>
            <div style={{ fontSize: 15, fontWeight: 700, color: '#12222F' }}>{sale.name}</div>
            <div style={{ fontSize: 12, color: '#5A6B7A' }}>{sale.region} · {sale.principal}</div>
          </div>
        </div>
        <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          <div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', border: '1px solid #DDE4EA', borderRadius: 8, overflow: 'hidden' }}>
              <span style={{ padding: 12, background: '#F4F7F9', color: '#7A8894', fontWeight: 700, fontSize: 14 }}>$</span>
        ...
        <div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', border: '1px solid #EEF2F5', borderRadius: 8, overflow: 'hidden', background: '#F4F7F9' }}>
            <span style={{ padding: 12, background: '#EBF0F3', color: '#7A8894', fontWeight: 700, fontSize: 14 }}>$</span>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px', background: '#F8FAFB', borderRadius: 8 }}>
          <div>
            <div style={{ fontSize: 13.5, fontWeight: 700, color: '#33475A' }}>Total job revenue</div>
            <div style={{ fontSize: 11.5, color: '#8695A2' }}>Added to this month's sales</div>
          </div>
